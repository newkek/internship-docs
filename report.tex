...
\end{itemize}
The threshold mentioned above can be a constant value, declared at \verb;Cluster;'s initialization. Or on a more advanced usage, the threshold can be dynamic, and paired with the \verb;PerHostPercentileTracker;. That way, users can easily detect a sudden latency happening on the network, maybe due to congestion, router failures, etc.
...
I/O in \emph{Netty} is exposed to users through \verb;Channel;s. When data is arriving on a \verb;Channel; (sending a packet, or receiving a packet), a callback handler is triggered to process the information on or to the \verb;Channel;. Executing a callback is creating a \verb;EventLoop; object, which can be viewed as the same as a \verb;Runnable; Java task. \verb;EventLoop;s are created by an \verb;EventLoopGroup;. The \verb;EventLoopGroup; is an entity managing clusters of threads (equivalent as a Java \verb;Executor;), that binds an \verb;EventLoop; to a thread, enqueues \verb;EventLoop;s and execute them on available threads. Hence there is a 1-n relationship between Thread and \verb;EventLoop;.\\
